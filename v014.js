...
(()=>{
  ...
  function buildReleaseHtml(snapshot){
    ...
    for(let i=0;i<experiments.length;i+=2){
      ...
      pair.forEach((item,index)=>{cards+='<article class="experiment"><div class="eyebrow">실험 '+Number(item.num||i+index+1)+'. '+htmlText(item.kicker||('실험 '+(i+index+1)))+'</div><h2>'+htmlText(item.title||'')+'</h2><p class="date">'+htmlEsc(item.date||'')+'</p><div class="media">'+mediaMarkup(item)+'</div><dl><div><dt>한 줄 요약</dt><dd>'+htmlText(item.summary||'')+'</dd></div><div><dt>질문</dt><dd>'+htmlText(item.question||'')+'</dd></div><div><dt>문제 상황</dt><dd>'+htmlText(item.context||'')+'</dd></div><div><dt>시도</dt><dd>'+htmlText(item.tried||'')+'</dd></div><div><dt>막힌 지점</dt><dd>'+htmlText(item.friction||'')+'</dd></div><div><dt>바꾼 점</dt><dd>'+htmlText(item.applied||'')+'</dd></div><div><dt>배운 점</dt><dd>'+htmlText(item.learned||'')+'</dd></div><div><dt>더 시도해 보면 좋을 것</dt><dd>'+htmlText(item.next||'')+'</dd></div></dl></article>'});
