...
(()=>{
  ...
  async function publishVersion(){
    let snapshot=null;try{const database=await db();snapshot=await new Promise((ok,no)=>{const request=database.transaction(SNAPSHOT_STORE).objectStore(SNAPSHOT_STORE).get(SNAPSHOT_KEY);request.onsuccess=()=>ok(request.result||null);request.onerror=()=>no(request.error)})}catch{}if(!snapshot)try{snapshot=JSON.parse(localStorage.getItem(SNAPSHOT_KEY)||'null')}catch{}
